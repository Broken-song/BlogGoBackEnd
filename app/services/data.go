...
func (dataService *dataService) GetDataByPage(params request.DataGet) DataResponse {
	...
	if order == 0 {
		...
		global.App.DB.Raw("SELECT * FROM tb_blogs ORDER BY updated_at DESC LIMIT ? OFFSET ?", limit, (page-1)*limit).Find(&tbblog)
	} else if order == 1 {
		//global.App.DB.Find(&tbblog, "ORDER BY updated_at ASC LIMIT ? OFFSET ?", limit, (page-1)*limit)
		global.App.DB.Raw("SELECT * FROM tb_blogs ORDER BY updated_at ASC LIMIT ? OFFSET ?", limit, (page-1)*limit).Find(&tbblog)
	}
	var count int64
